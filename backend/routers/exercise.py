...
def create_exercise(request: ExerciseCreate,
...
def update_one_exercise(id: int, request: ExerciseUpdate, db: Session = Depends(get_db), get_current_user: UserView = Depends(get_current_user)):
    return ExerciseRepository.update_one(id, request, db)


@router.delete("/", status_code=status.HTTP_204_NO_CONTENT)
def delete_exercise(id: int, db: Session = Depends(get_db), get_current_user: UserView = Depends(get_current_user)):
