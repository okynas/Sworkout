...
def get_one_workout(workout_id: int, db: Session, current_user: User):
  ...
  session = db.query(Session).filter( Session.workout_id == workout_id and Session.user_id == user_to_check.user_id ).first()
...
def create(request: SessionCreate, db: Session, current_user: User):
  ...
  if not workout_find:
    raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"Workout with id {request.workout_id} does not exists")
...
def delete(workout_id: int, db: Session, current_user: User):
  ...
  session = db.query(Session).filter(Session.workout_id == workout_id)
  if not session.first():
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Session with workout with id: {id} not found")
