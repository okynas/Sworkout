...
def create(request: WorkoutCreate, db: Session):
  if not (request.set or request.weight or request.repetition):
    raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY , detail=f"Set, Weight and Repetition of workouts is required")
  ...
  if workout_find:
    raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"Workout with exercise id {request.exercise_id} already exists")
